...
equipment = L{
    ...
    [23373] = 'Azimuth Gaiters +2',
    [23708] = 'Azimuth Gaiters +3',
...
function prerender()
    ...
    if nexttime + del <= curtime then
        ...
        if geo and not luopan and spell_recasts[geo.id] <= 0 and play.vitals.mp >= geo.mp_cost and (geo.targets == 5 or target and target.hpp > 0) then
            ...
            -- ATTRITION
            elseif settings.attrition and abil_recasts[347] and abil_recasts[347] <= 0 then
                use_JA('Ecliptic Attrition','<me>')
            else
...
function addon_command(...)
    ...
    if commands[1] then
        ...
        elseif commands[1] == 'attrition' and commands[2] then
            if commands[2] == 'on' then
    ...
    if id == 0x028 then
        ...
        if packet.Category == 4 then
            ...
            if packet.Param >= 768 and packet.Param <= 797 then
                ...
                if get_equip('legs') == 'Bagua Pants +1' then dur = dur + 15 end
                if get_equip('legs') == 'Bagua Pants +2' then dur = dur + 18 end
                if get_equip('legs') == 'Bagua Pants +3' then dur = dur + 21 end
                if get_equip('feet') == 'Azimuth Gaiters' then dur = dur + 15 end
                if get_equip('feet') == 'Azimuth Gaiters +1' then dur = dur + 20 end
                if get_equip('feet') == 'Azimuth Gaiters +2' then dur = dur + 25 end
                if get_equip('feet') == 'Azimuth Gaiters +3' then dur = dur + 30 end
